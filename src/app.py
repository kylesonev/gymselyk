...
with st.form("form_aluno"):
    ...
    with col1:
        sexo = st.selectbox(
            "Sexo", ["Selecione", "Masculino", "Feminino"], index=0)
    ...
    with col2:
        ...
        altura = st.number_input(
            "Altura (cm)", min_value=100, max_value=250) / 100
        dias_disponiveis = st.selectbox(
            "N° dias disponível", ["3", "4", "5", "6"])
                    ...
                    for i, row in df_planilha.iterrows():
                        ...
                        if pd.notna(video_url) and str(video_url).strip() != "":
                            with st.expander(f"{row['ex']}"):
                                if str(video_url).lower().endswith(".gif") or "giphy" in str(video_url):
                                    st.image(
                                        video_url, use_container_width=True)
                                else:
                                    st.video(video_url)
    else:
        st.warning(
